...
Rails.application.routes.draw do
  ...
  get "/v1/(*path)", to: redirect { |params, request| "/api#{request.path}?#{request.query_string}" }
  get "/v2/(*path)", to: redirect { |params, request| "/api#{request.path}?#{request.query_string}" }
  get "/api/:version/commodities/:id", to: redirect { |_, request| request.url.gsub('commodities', 'chapters').gsub('00000000', '') }, constraints: { id: /\d{2}00000000/ }
  get "/api/:version/commodities/:id", to: redirect { |_, request| request.url.gsub('commodities', 'headings').gsub('000000', '') }, constraints: { id: /\d{4}000000/ }
